...
public:
  ...
  //
  sc_core::sc_mutex m_mutex_0, m_mutex_1;
                           ...
                           sc_core::sc_time& t)
  {
	...
	if(portId == 0) m_mutex_0.lock();
	else if(portId == 1) m_mutex_1.lock();
	
	//printf("portId_bus_64 = %d\n", portId);
    (*decodeSocket)->b_transport(trans, t);
	
	if(portId == 0) m_mutex_0.unlock();
	else if(portId == 1) m_mutex_1.unlock();
  }
