...
public:
  ...
  //
  sc_core::sc_mutex m_mutex_0_16, m_mutex_1_16;
                           ...
                           sc_core::sc_time& t)
  {
	...
	if(portId == 0) m_mutex_0_16.lock();
	else if(portId == 1) m_mutex_1_16.lock();
	
	//printf("portId_bus_16 = %d\n", portId);
    (*decodeSocket)->b_transport(trans, t);
	
	if(portId == 0) m_mutex_0_16.unlock();
	else if(portId == 1) m_mutex_1_16.unlock();
  }
